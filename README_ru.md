...
|---|---|---|---|---|---|
| Полнотекстовый поиск | ✅ до 50 результатов, клиентские фильтры | ❌ поисковой тулзы нет | ❌ | ✅ до 10 результатов | ❌ |
| Страницы: create / update / append / delete + recover | ✅ всё | частично — нет append / recover; есть частичное редактирование заменами текста | ✅ всё | частично — нет append / recover | частично — нет recover |
...
| Комментарии, загрузка вложений | ✅ | комментарии ✅ / загрузка ❌ (вместо неё скачивание + превью) | ✅ | ❌ | ❌ |
...
- **Фильтры работают на сервере, до лимита** — отфильтрованный поиск не теряет из-за него совпадения: `slug_prefix` (фильтр по разделу, глубокие префиксы вроде `tech-doc/ml` работают), `result_type` (`page`/`file`) и интервалы дат `created_between`/`modified_between` (обе границы обязательны — открытые API отклоняет).
