...
- Поиск **пока только глобальный** — фильтры `slug_prefix` и `result_type` применяются на клиенте после получения, поэтому сочетайте их с `limit=50`, чтобы не терять совпадения (новые серверные фильтры API — в [роадмапе](ROADMAP.md)).
