...
поиск, страницы, комментарии, вложения и динамические таблицы («гриды») — **32 тулзы**
...
32 тулзы. Все write-тулзы исчезают при `WIKI_READ_ONLY=true`.
...
| `page_download_attachment` | Прочитать содержимое вложения прямо в диалог (никуда не сохраняется) — текст как текст, мелкий бинарь base64-блобом. Задумана для текстовых вложений: конфиги, CSV, логи. Кап 128 KiB бережёт контекстное окно модели; что больше — отказ ещё до передачи, с отсылкой на `download_url` из `page_get_attachments` — для картинок и PDF это правильный путь в любом случае |
...
| Комментарии, загрузка вложений | ✅ вкл. удаление и скачивание вложений | комментарии ✅ / загрузка ❌ (вместо неё скачивание + превью) | ✅ | ❌ | ❌ |
...
поверхностью: полнотекстовый поиск, типизированные схемы входа *и* выхода у всех 32 тулз,
