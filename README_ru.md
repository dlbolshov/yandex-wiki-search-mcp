...
поиск, страницы, комментарии, вложения и динамические таблицы («гриды») — **33 тулзы**
...
33 тулзы. Все write-тулзы исчезают при `WIKI_READ_ONLY=true`.

### Поиск и чтение (10)

| Тулза | Что делает |
|---|---|
| `page_search` | Полнотекстовый поиск по всей Вики (страницы и файлы), ранжированные результаты с фрагментом текста; серверные фильтры, а в режиме `highlight` — курсорная пагинация до ~100 результатов (иначе — до 50 за один вызов) |
...
| `page_read_attachment` | Прочитать содержимое вложения прямо в диалог (никуда не сохраняется) — PNG/JPEG/GIF/WebP нативным image-блоком, который клиенты с vision рендерят, текст как текст (включая SVG: это XML, а image-блок, который vision API не декодирует, роняет следующий вызов хоста), прочий бинарь base64-блобом. Формат определяется по magic-байтам файла, а не по заявлению провода. Кап бережёт контекстное окно модели: 128 KiB для текста/бинаря, 2 MiB для картинок; что больше — отказ с отсылкой на `page_download_attachment` или `download_url` из `page_get_attachments` |
...
| `user_get_current` | Кто я — `username` и `home_cluster` (слаг личного раздела вызывающего) |
...
| `page_update` | Обновить заголовок и/или полное содержимое; поставить или снять редирект на другую страницу |
| `page_edit` | Правка содержимого точными заменами текста без пересылки всей страницы; ненайденное или неоднозначное совпадение роняет вызов до того, как что-либо записано; запись идёт с `allow_merge`, поэтому параллельная правка сливается, а не затирается |
...
| `page_delete_comment` | Удалить комментарий; возвращает обновлённое число комментариев страницы |
| `page_delete_attachment` | Удалить вложение со страницы |
...
| `page_download_attachment` | Скачать вложение в локальный файл — потоком на диск без ограничения размера, в диалог ничего не попадает. Пишется атомарно (`.part` → fsync → rename), существующий файл не перезапишет без явного `overwrite`, права — как при обычной записи (`0666 & ~umask`, никогда не исполняемый); при замене сохраняются права заменяемого файла. Fsync каталога, который делает само переименование устойчивым к падению, и наследование прав — только на POSIX. При OAuth гейтится так же, как `page_upload_attachment` |
...
| Гриды: write-тулзы | ✅ 11 | ✅ 12, вкл. апдейт колонок и pin/цвет строк | ✅ 11 | ❌ только чтение | ✅ 11, вкл. clone |
| Комментарии, загрузка вложений | ✅ вкл. удаление, превью картинок в диалоге и скачивание на диск | комментарии ✅ / загрузка ❌ (вместо неё скачивание + превью) | ✅ | ❌ | ❌ |
...
`page_search` оборачивает эндпоинт `POST /v1/search` — тот же бэкенд, что у строки
поиска в веб-интерфейсе Вики; он был недокументирован, пока в августе 2026 Яндекс не
опубликовал [справочник](https://yandex.ru/support/wiki/ru/api-ref/search/search__search).
Сначала ищите, потом открывайте результат через `page_get` по его `slug`.

- Два режима на проводе. По умолчанию: до **50** результатов за один вызов (`limit`
  ограничивается 1–50; остальное API отклоняет) и без пагинации — курсоры в ответе
  всегда `null`. С `highlight=true`: страницы жёстко режутся до **10** результатов
  независимо от `limit`, совпадения приходят обёрнутыми в `<em>`, а `cursor`
  (номер страницы, который ответ возвращает в `next_cursor`) листает до **~100**
  результатов. Набор закончился, когда `results` пришёл пустым или `next_cursor`
  равен `null` на непустой странице — за концом набора `next_cursor` продолжает
  расти над пустыми страницами, так что сам по себе он не означает «есть ещё».
- **Фильтры работают на сервере, до лимита** — отфильтрованный поиск не теряет из-за него совпадения: `slug_prefix` (фильтр по разделу, глубокие префиксы вроде `tech-doc/ml` работают), `result_type` (`page`/`file`), `authors` (владельцы страниц по `uid`/`cloud_uid` — свои даёт `user_get_current`, и «найди мои страницы про X» превращается в два вызова) и интервалы дат `created_between`/`modified_between` (обе границы обязательны — открытые API отклоняет).
...
- `content` — это **фрагмент ~510 знаков, не страница и не выжимка**: он вырезан по месту совпадения, слов запроса внутри может не быть, а переводы строк и табы — собственная разметка страницы (ячейки таблиц приходят через табы), а не разделители между кусками. С `highlight=true` совпадения оборачиваются в теги `<em>`. Прежде чем отвечать по нему, прочитайте страницу через `page_get`. У результатов-`file` поле пустое.
...
В этом режиме `page_upload_attachment` и `page_download_attachment` не регистрируются:
они читают и пишут файлы на машине, где запущен сервер, а в общем деплое это не
машина вызывающего.
...
- **Wiki API не проверяет OAuth-скоупы** (перепроверено 2026-08-11, уже после того как Яндекс их задокументировал — см. [docs/api-notes_ru.md](docs/api-notes_ru.md)): токен с `wiki:read` по-прежнему может писать, поэтому полагайтесь на read-only режим, а не на скоупы.
...
Вики-API дрейфует (эндпоинт поиска уже однажды молча сменил контракт — ещё в бытность
недокументированным) — `scripts/contract_sweep.py` перепроверяет каждый метод клиента на живой
...
поверхностью: полнотекстовый поиск, типизированные схемы входа *и* выхода у всех 33 тулз,
...
он первым обнаружил тогда ещё недокументированный эндпоинт `POST /v1/search`
(справочник по нему Яндекс опубликовал только в августе 2026) и сообщил, что
