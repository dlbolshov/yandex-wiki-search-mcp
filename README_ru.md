...
| `page_read_attachment` | Прочитать содержимое вложения прямо в диалог (никуда не сохраняется) — картинки нативным image-блоком, который клиенты с vision рендерят, текст как текст, мелкий бинарь base64-блобом. Кап до передачи бережёт контекстное окно модели: 128 KiB для текста/бинаря, 1 MiB для картинок; что больше — отказ с отсылкой на `page_download_attachment` или `download_url` из `page_get_attachments` |
...
| `page_download_attachment` | Скачать вложение в локальный файл — потоком на диск без ограничения размера, в диалог ничего не попадает; существующий файл не перезапишет без явного `overwrite`. При OAuth гейтится так же, как `page_upload_attachment` |
