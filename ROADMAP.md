...
## M7 — Документированный API (v1.3.0, план согласован 2026-08-11)
...
- [x] `page_search`: серверные фильтры + подсветка (M) — сделано 2026-08-11:
  ...
  - `filters.authors` и `show_obsolete` задокументированы, но не пробованы — решить,
    выставлять ли
- [ ] `page_delete_comment`: `DELETE /pages/{id}/comments/{cid}` — 200 +
      обновлённый `comments_count` (S)
- [ ] `page_download_attachment`: `GET /pages/{id}/attachments/{fid}/download` —
      200 + байты (M). Решить формат выдачи: текст → текст, бинарь → base64 или отказ
      со ссылкой; под `OAUTH_ENABLED` безопасно — контент идёт в ответе, файловая
      система сервера не участвует (в отличие от upload)
- [ ] `page_delete_attachment`: `DELETE /pages/{id}/attachments/{fid}` — 204 (S)
- [ ] Редирект в `page_update`: `redirect={"page": {"id": N}}` ставит,
      `{"page": null}` снимает; читается через `page_get(fields=["redirect"])` —
      живьём проверено (S)
- [ ] `user_get_current`: `GET /users/me` — `username`, `home_cluster` (слаг личного
      ...
      `users/<login>/...`-слагов без угадывания
- [ ] Кандидат, решить при реализации: `page_edit` — частичное редактирование
      заменой текста (M). Это НЕ эндпоинт API (в доке только полный update + append) —
      официальный MCP делает read-modify-write у себя внутри. Плюсы: экономит токены
      на больших страницах, падает при ненайденном/неоднозначном совпадении вместо
      тихой перезаписи. Минус: у страниц нет ревизий — гонка last-write-wins между
      чтением и записью неустранима на этом API в принципе
