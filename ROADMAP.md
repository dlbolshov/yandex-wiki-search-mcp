...
- [ ] `page_search`: серверные фильтры + подсветка (M)
  ...
  - решить при реализации: судьба клиентских `slug_prefix`/`result_type` —
    проверить, берёт ли `cluster` глубокий префикс (`a/b/c`) или только первый
    сегмент; `filters.type` закрывает `result_type` целиком
