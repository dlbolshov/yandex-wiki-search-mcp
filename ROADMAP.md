...
- [x] `fetch_all: bool` для курсорных тулзов: цикл в tool-слое по `next_cursor`, жёсткий потолок
      ...
      (посылка устарела 2026-08-25: highlight-режим отрастил настоящие курсоры, но
      решение в силе — стоп-условие поиска нетривиальное и прописано в описании тулзы)
...
- 2026-08-21 (ревью attachments, третий заход): каждый файл, скачанный на
  Windows, был бы битым — голый `os.open` открывает `.part` в текстовом режиме
  CRT (`\n` → `\r\n`, `write()` об этом не сообщает); теперь `O_BINARY`, как
  делает сам tempfile, замена которого и внесла регрессию. Лок с реестром из
  второго захода не покрывал `_write_all` — отмена закрывала fd на event loop,
  пока осиротевший воркер ещё писал (инструментация поймала EBADF, а при ином
  исходе гонки байты уходили в чужой fd); вместо лока — выделенный
  однопоточный executor на вызов: все ФС-шаги идут в порядке submission,
  cleanup встаёт в очередь за незавершённой записью. Пул закрывается через
  `shutdown(wait=False)` и намеренно не через `with`: выход из контекст-менеджера
  зовёт `shutdown(wait=True)` на треде цикла, и отмена с незавершённой записью
  морозила весь сервер до её конца — замерено 951 мс против 1 с заглушки, а на
  зависшем маунте это неограниченно. `wait=False` очередь всё равно дренирует,
  так что `.part` убирается — просто уже не на цикле (16 мс вместо 951). Ошибка fsync директории после rename логируется, а не роняет уже
  доставленный файл; `fchmod` (EPERM на vfat/SMB) завёрнут внутри `_open_part`;
  `ResponseTooLarge` оформляется в тул-слое с подсказкой (имя
  `page_download_attachment`/`download_url` — с учётом гейтов регистрации);
  `.part`-имя бюджетируется и против MAX_PATH. Поверх ревью: виндовый
  бюджет не вычитал суффикс (`.part`-путь вылезал за MAX_PATH на 14 символов),
  проба цели после рерайта звалась дважды, а `with ThreadPoolExecutor` блокировал
  цикл на отмене — поправлено. 703 теста, 100%
  покрытие (платформенные ветки — под `# pragma`).
- 2026-08-21: v1.4.0 выпущен — вложения в обе стороны (`ImageContent` для
  картинок в `page_read_attachment`, новый `page_download_attachment` на
  диск), поверхность 32 → 33 тулзы. Четвёртый заход ревью перед мержем:
  100%-гейт стал отдельным CI-шагом только на ubuntu — гейт на всех ОС
  требовал исключать POSIX-only ветки из измерения везде, и исключение
  целиком `_fsync_directory` скрыло, что fsync каталога не проверялся ни
  одним тестом (подмена тела на `return` оставляла сьют зелёным). Под теми
  исключениями нашлись и дефекты: MAX_PATH-бюджет давал ровно 260 при
  лимите CreateFileW в 259 (NUL считается), слишком длинный родитель теперь
  отказывается с причиной; hardlink-fallback узнал ENODEV/ENOTSUP — маппинг
  ERROR_NOT_SUPPORTED в CPython, без которого fallback для FAT/exFAT/SMB не
  срабатывал именно на Windows; `_drain` не запрашивает больше
  `min(limit+1, 1 MiB)`, чтобы малый потолок не материализовывал целый чанк;
  неубираемый `.part` после успешного link и cleanup, который нельзя
  запланировать при teardown интерпретатора, — warning вместо провала уже
  доставленного файла. 712 тестов, 100% покрытие (стейтменты и ветки).
- 2026-08-26: v1.5.0 выпущен — курсорная пагинация поиска. Между 2026-08-11
  и 2026-08-25 провод молча отрастил второй режим поиска за `highlight=true`:
  страницы жёстко по 10 результатов независимо от `limit`, настоящие строковые
  курсоры в ответе, обход до ~100 результатов, `order_by` работает (не
  выставлен). `page_search` получил `cursor` (1–500); клиент отказывает
  `cursor` без `highlight` (провод молча ответил бы первой страницей), в
  описании тулзы — честное стоп-условие (пустой `results` или `null` на
  непустой странице: за концом набора `next_cursor` растёт над пустыми
  страницами до потолка 500). Свип пинит оба режима (BROKEN только на
  полном реплее — slug + `next_cursor`, свопы соседей в ранжировании
  легитимны), docs_probe меряет курсор в обоих. api-notes переписаны вокруг
  сплита; `[tool.uv] required-version` убрал варнинги setup-uv в CI.
  718 тестов, 100% покрытие.
- 2026-09-21: v1.5.1 выпущен — патч без изменений в поверхности тулз и в
  поведении сервера. Еженедельный дрейф-чек покраснел (зелёный 2026-09-14):
  между прогонами каждый ответ со страницей отрастил топ-левел
  `active_revision`, а `append-content` ещё и `actuality` — оба `null` на любой
  пробованной странице, в референсе ни одного, а документированный
  `revision_id` у `GET /pages/{idx}` отвечает 404 на любой id: хранилище
  ревизий проступает на проводе пустым. Модели не тронуты (`extra="ignore"` и
  так отбрасывал оба ключа); свип терпит ровно эти два ключа, пока они `null`
  (`NULL_UNTIL_LIVE`), — первое значение снова красит прогон. Записано в
  api-notes (EN/RU). `contract_sweep.py` берёт scratch-slug из `SWEEP_SLUG`
  (окружение или `.env`), если позиционный аргумент опущен; `.env.example`
  документирует его в dev-блоке. 718 тестов, 100% покрытие.
