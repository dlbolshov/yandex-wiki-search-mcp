...
- [x] `page_download_attachment` — скачивание вложения в локальный путь, обратная
      ...
      сервере — чужая ФС) и не read-only — сделано 2026-08-19
- [x] Картинки в `page_read_attachment` через `ImageContent` (M): для `image/*`
...
- 2026-08-19: оба кандидата после 1.3.0 закрыты (войдут в следующий минорный
  релиз). Проба провода:
  download-эндпоинт декларирует точный пофайловый `Content-Type`, но не шлёт
