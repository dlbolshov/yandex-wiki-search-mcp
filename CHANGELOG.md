...
### Fixed
- `page_download_attachment` no longer buffers a whole attachment before deciding it is too big. The ceiling moved into `WikiClient._request` as a `max_bytes` argument checked against `Content-Length` before any body is read (and against the stream itself for a chunked response), so an oversized file never lands in the process at all — it raises the new `ResponseTooLarge`. The capped read drains the stream in a loop, because aiohttp's `StreamReader.read(n)` returns whatever is already buffered, up to `n` — a single call would silently truncate a body that arrives in several network chunks. The download is also no longer retried: it is a GET, but repeating it re-transfers the whole file for nothing. An error response on a capped request is truncated at a separate ceiling rather than refused — a `ResponseTooLarge` there would replace the API's own explanation of the failure with a complaint about its size — and the refusal message now distinguishes its two paths, since only the `Content-Length` one leaves the body genuinely unread
- The inline ceiling dropped from 1 MiB to 128 KiB, and the tool now returns an `EmbeddedResource` instead of a pydantic model. Both fix the same thing: a megabyte of base64 is ~1.4M characters, and a model result is shipped **twice** (once as `structured_content`, once as the spec-recommended text mirror), so the old cap authorized ~2.7 MiB of transport in a guard whose stated job was protecting the model's context. A content block is shipped once — `WikiMCPServer.call_tool` already left non-text blocks alone in anticipation of exactly this tool
- `page_edit` writes back with `allow_merge=True` and exposes `allow_merge`/`is_silent`. The tool is a read-modify-write with an LLM composing replacements inside the window; `page_append_content`'s anchor fallback — the only other read-modify-write here — already passed the flag, so a concurrent edit was being overwritten where the same shape of write elsewhere was merged
- `page_edit` is no longer annotated `idempotent_hint=True`. A replacement whose `new_text` contains its own `old_text` (`"## Setup"` → `"## Setup\n\n…"`) still matches its own output, so a client retrying a lost response on the strength of the hint would insert twice
- `page_search` rejects `authors: []` and a blank `uid`/`cloud_uid` instead of silently searching the whole Wiki. An empty filter was dropped on the wire, so "find Maria's pages" whose identity lookup came back empty returned the entire organization — the trap `slug_prefix` already refused, in the more dangerous direction
- `filters.cluster` is normalized in `WikiClient.page_search` rather than in the tool. Probing it live (2026-08-18) settled three open questions: it matches on **path segments** (`a/b` does not leak `a/bc`) and **includes the cluster page itself** — both guarantees the tool advertises hold — but the value is matched **literally**, so mixed case or a stray leading/trailing slash answers 200 with zero results while `GET /pages?slug=` resolves all three spellings. A direct client caller was silently getting nothing
- `page_delete_comment` returns `{page_id, comment_id, deleted}` alongside `comments_count`, and guards a non-object body. `comments_count` was the model's only field, so an empty or renamed body dumped to `{}` — a successful call carrying no evidence the comment was gone — and a scalar body raised a bare `ValidationError` past the `WikiError` hierarchy
- `page_delete_attachment` merges the response body into its acknowledgment instead of discarding it, so a JSON body the API starts sending reaches the model where the contract sweep can see it. Its docstring claimed the `grid_delete` pattern; now it follows it
- `_occurrence_lines` steps past the whole match instead of one character. It walked **overlapping** occurrences while the count reported beside it comes from `str.count`, which does not — so `page_edit`'s ambiguity error could list more line numbers than occurrences (`"--"` inside a `"----"` rule), and the "and more" marker was computed from a different tally than the list it annotated
- `page_download_attachment` no longer treats "decodes as UTF-8" as "is text". UTF-16LE without a BOM — an ordinary Windows-exported `.txt`/`.csv` — decodes cleanly because every byte is a valid code point, and arrived as NUL-riddled mojibake labelled text; a NUL check now sends it as a blob
- The contract sweep's attachment round-trip verdict is a `REPORT` row rather than a note. `check()` only escalates status for `BaseModel`/`list` results, so an assertion smuggled into `note_from_result` could never fail the sweep — and the `b.decode()` inside it ran outside `check()`'s `try/except`, so a non-UTF-8 body would have killed the job with a traceback instead of recording a verdict
- The sweep's `KNOWN_DROPPED` no longer blinds `user_get_current`. `identity`/`uid`/`cloud_uid` are suppressed because they ride along on every user reference — but `/users/me` **declares** them, and they are what the `authors` filter is built from, so drift on the one endpoint that owns them was being subtracted before it could be reported. Three silent `if` guards became `SKIP` rows for the same reason, and a new check pins the `cluster` segment-boundary contract live
...
- `SearchAuthor` extends `UserIdentity` instead of re-declaring its two fields; the three tool-only models moved out of the wire-shape module into the tool that owns them (`AttachmentDownloadResult` is gone entirely, replaced by the embedded resource), and `PageEditResponse.edits_applied` went with them — it was always `len(replacements)`, since a replacement that does not apply fails the whole call
- Stale counts and claims: `server.py` said 31 tool schemas and both architecture docs said 21 write tools (it is 32 and 22); `CLAUDE.md` still described search excerpts as unhighlighted after `highlight` shipped, and its capability list predated the five new tools; a read-only-mode assertion checked for a capability string the same change had renamed away
...
- `page_search` filters by author: `authors` takes a list of `{uid, cloud_uid}` identities matched against the page owner — either field alone works, several entries OR together, and `user_get_current` supplies the caller's own ids, turning "find my pages about X" into one call after another (verified live 2026-08-18; an unknown identity yields an empty result, an identity without either id is rejected by the schema because the wire silently ignores it)
- `page_download_attachment` — reads an attachment's content inline: UTF-8 bytes arrive as text, anything else base64-encoded, both alongside `size_bytes` and the id pair. Capped at 1 MiB — the cap protects the model's context window, not the wire — and an oversized file is refused with a pointer to the `download_url` that `page_get_attachments` already returns. Registered as a read tool, so it survives `WIKI_READ_ONLY=true`; under OAuth it is safe by construction — the bytes travel in the response, the server's filesystem is never involved (unlike upload, which stays gated). A missing attachment on this endpoint is a 404 with a **placeholder GIF body** instead of the JSON error envelope, so the client maps it to the new `AttachmentNotFound` itself
...
- `page_delete_comment` — deletes a comment and returns the page's updated `comments_count`. Comment ids come from `page_get_comments`
- `page_delete_attachment` — deletes an attachment; the API answers a bare 204, so the tool returns a constructed `{page_id, file_id, deleted}` acknowledgment (the `grid_delete` precedent). The description warns that any file macro referencing the attachment stays behind, broken
...
- The weekly contract sweep covers the five new client methods: comment deletion, attachment download (asserting the payload round-trips byte-for-byte) and deletion, the set → read back → clear redirect cycle, `user_get_current`, plus a filtered-and-highlighted search for the sweep's own fixtures by their author — all green on the 2026-08-18 run
