...
- The 100% coverage gate now runs on the ubuntu leg alone; every leg still runs every test it can. Demanding 100% on all nine cells could only be met by excluding the POSIX-only arms from measurement everywhere, and a whole-function exclusion on `_fsync_directory` turned out to hide that *no* test asserted the directory fsync happens at all — replacing its body with `return` kept the suite green at 100%. The exclusions are back to the Windows-only arms, and `test_a_finished_download_flushes_the_directory_entry` now pins the durability guarantee the way `test_a_full_disk_at_fsync_is_a_wiki_error` pins the data fsync
