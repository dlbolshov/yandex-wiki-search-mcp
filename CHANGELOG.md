...
- `page_read_attachment` no longer treats "decodes as UTF-8" as "is text". UTF-16LE without a BOM — an ordinary Windows-exported `.txt`/`.csv` — decodes cleanly because every byte is a valid code point, and arrived as NUL-riddled mojibake labelled text; a NUL check now sends it as a blob
