...
### Changed
- `page_search`'s tool description, its `highlight` argument description, and the server instructions now describe the two wire modes. The 1.3.0 wording "there is no deeper pagination" stopped being true when the highlight mode grew cursors, and `highlight` was never just markup after that: it also caps every page at 10 results, which a caller passing `limit=50` with `highlight=true` had been silently getting since the backend split

### Internal
- The contract sweep pins both search modes: default-mode cursors must stay null (a cursor showing up there means the mode split healed and the tool description undersells the endpoint), and the highlight mode must actually page — page 1 over the sweep's own fixtures at `limit=1` must hand out a cursor, following it must return a different result, and both checks degrade to SKIP rows rather than silence when the search index has not caught up with the fixtures
