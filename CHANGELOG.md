...
### Internal
- The contract sweep's attachment round-trip now also asserts the download's `mimetype` and covers the download-to-disk path end-to-end: bytes on disk equal bytes uploaded, no `.part` leftovers
...
- `test_tilde_expands_to_the_home_directory` sets `USERPROFILE` alongside `HOME`: `ntpath.expanduser` ignores `HOME`, so on the three Windows CI legs the test wrote its fixture into the runner's real profile directory and then failed the assert
...
- A failure after the rename no longer reports the download as failed: past that point the bytes are in place and correct, so an unflushable directory is logged rather than raised. It used to tell the caller to retry a file that was already complete — and, with `overwrite`, one that had already replaced theirs
...
- Windows: the `.part` is opened with `O_BINARY` (without it the CRT's text mode rewrote every `\n` in the body, corrupting every image, PDF and archive saved there — a regression from replacing `tempfile`, which sets the flag itself); a parent path component that is a regular file is now detected there too, where Win32 reports it as ENOENT rather than ENOTDIR; and the `.part` name is budgeted against `MAX_PATH`, not just `NAME_MAX`
...
- The platform-conditional branches carry `# pragma: no cover` with their reasons — both halves: the Windows-only arms, and the POSIX-only ones (the fchmod inheritance, the directory fsync and its warning, the `NotADirectoryError` probe arm) whose tests skip on Windows and were failing that leg's 100% gate. Each OS leg can only execute its own half, so the excluded side is always the one the other legs' tests still pin
